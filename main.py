...
async def analyze(
    ...
    # Run rules-based analysis (provisioners + EC2NodeClass + relationships)
    issues = run_analysis(all_provisioners, all_nodeclasses)

    # Store for download endpoint
    global LAST_ISSUES
    LAST_ISSUES = issues

    summary = generate_summary(all_provisioners, issues, all_nodeclasses)
